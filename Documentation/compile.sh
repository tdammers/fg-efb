...
for source_img in $source_images; do
    ...
    case "$(basename "$dest_img")" in
        ...
        paperwork-map.jpg)
            convert "$source_img" -crop 1080x720+50+190 -quality 95 "$dest_img"
            ;;

        home-screen-annotated.jpg)
            convert "$source_img" -quality 95 "$dest_img"
            ;;

        *)
            convert "$source_img" -crop 720x1010+150+32 -quality 95 "$dest_img"
            ;;
    esac
done
(
